---
title: "Productive Enterprise Web Development with ExtJS and Clear Data Builder"
date: 2013-02-05
author: "Viktor Gamov"
tags: ["java", "extjs", "sencha", "cdb", "tutorial"]
slug: "prouctive-enterprise-web-dev-with-cdb"
---
:toc: macro
...
One of the most feature-complete frameworks is http://www.sencha.com/products/extjs[ExtJS] from Sencha, but its learning curve is a bit steep. Our company, http://faratasystems.com[Farata Systems], has developed an open source software Clear Toolkit for ExtJS. 
Clear Toolkit includes an Eclipse plugin called Clear Data Builder (CDB). 
It is a productivity tool - a code generator - that can create a CRUD application for you in no time. This application will have HTML/JavaScript/ExtJS client and Java-based server. In this article, you will learn how jumpstart development of such Web applications.
...
- Clear Runtime - Java components implement server side part of `ChangeObject`, `DirectOptions` etc.

The phrase "to be more productive" means to write less code and produce the results faster. 
This is what CDB for. In this article, you will see how Clear Data Builder helps you to integrate the client side with the back end using the RPC style and how to implements data pagination for your application.

CDB distribution is available as a plug-in for Eclipse IDE. The current update site of CDB located http://cleartoolkit.com/downloads/plugins/extjs/cleardatabuilder/4.1.4/[here].  The current version is 4.1.4 (don't be surprised - this is a five-year-old code generator, and its previous versions were made for generating the UI for Adobe Flex framework). You can install this plug-in via +Install New Software+ menu in Eclipse IDE. The <<FIG-1>> shows how you can validate the plug-in installation. If you see "Clear Data Builder for Ext JS feature" in the list of Installed Software in your Eclipse IDE, you are good to go.
...
[source,java]
-----------------------------------------------------
// Person.java - source file no longer available
@JSClass
public class Person {
    @JSGeneratedId
    private int id;
...
public class PersonServiceImpl extends _PersonServiceImpl {
    ...
    @Override
    public void getPersons_doCreate(ChangeObject changeObject) { // <3>
    ...
    @Override
    public void getPersons_doUpdate(ChangeObject changeObject) { // <4>
        // update logic
    }

    @Override
    public void getPersons_doDelete(ChangeObject changeObject) { // <5>
        // delete logic
    }
}
----------------------------------------------------------------------
<1> We can use `java.util.ArrayList` as in-memory server side storage of the `Person` objects. In real world applications you'd use a database or other persistent storage.
<2> `getPersons` is our retrieve method (the R in CRUD).
<3> `doCreate()` is our create method (the C in CRUD).
<4> `doUpdate` is our update method (the U in CRUD).
<5> `doDelete` is our delete method (the D in CRUD).
...
[source,javascript]
-----------------------------------------------------
var myStore = Ext.create('Episode1.store.dto.PersonStore', {}); // <1>
...
[source,javascript]
-----------------------------------------------------
Ext.define('Episode1.controller.SampleController', {
    extend: 'Ext.app.Controller',
    stores: ['dto.PersonStore'],
    refs: [{ref: 'ThePanel', selector: 'simpleGrid'}], // <1>

    init: function() {
        this.control({
            '#load': { click: this.onLoad }
        });
    },

    onLoad: function() {
        this.getThePanel().getStore().load(); // <2>
    }
});
-----------------------------------------------------
...
public List<Person> getPersons() {
    ...
    int start = ((Double) DirectOptions.getOption("start")).intValue(); // <1>
    int limit = ((Double) DirectOptions.getOption("limit")).intValue(); // <2>
    limit = Math.min(start + limit, result.size()); // <3>
    DirectOptions.setOption("total", result.size()); // <4>
    return result.subList(start, limit); // <5>
}
----------------------------------------------------------------------
...
<5> Before returning the result, create a subset using `java.util.List.subList()` between `start` and `limit`.  
...
* https://github.com/Farata/ClearJS/wiki/Change-Object:-Synchronization-Keystone[Change Object: Synchronization Keystone]
